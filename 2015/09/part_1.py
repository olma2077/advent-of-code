...
def shortest_distance(graph, start, path):
    # backtracking approach with selecting a minimum possible path distance
    path.append(start)
    shortest_dst = 0

    # if not all locations visited
    if len(path) < len(graph):
        shortest_dst = float("inf")

        for next_loc, dst in graph[start]:
            if next_loc not in path:
                shortest_dst = min(
                    shortest_dst,
                    dst + shortest_distance(graph, next_loc, path)
                )
...
def solver(filename):
    ...
    return min([shortest_distance(graph, start, []) for start in graph])
